...
<header class="fixed top-0 left-0 right-0 z-50 border-b border-white/5">
  ...
  >
    ...
    <a
      href="/"
      class="flex items-center gap-2.5 font-bold text-white group"
      onclick={closeMobileMenu}
    >
      <div
        class="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600 text-xs font-black shadow-lg shadow-indigo-500/20 transition-shadow group-hover:shadow-indigo-500/40"
      >
      ...
      {#each navLinks as link}
        ...
        >
          ...
          {#if isActive(link.href)}
            <span
              class="absolute bottom-0 left-1/2 h-0.5 w-4 -translate-x-1/2 rounded-full bg-indigo-500"
            ></span>
          {/if}
        </a>
      {/each}
    ...
    <!-- Mobile Hamburger -->
    <button
      class="lg:hidden text-zinc-400 hover:text-white transition-colors"
  ...
  {#if mobileMenuOpen}
    <div
      class="lg:hidden border-t border-white/5 bg-zinc-950/95 backdrop-blur-xl"
    >
